"""Create or authenticate the deferred portable water-distance freeze decision."""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--check-only",
        action="store_true",
        help="Authenticate the existing append-only decision manifest.",
    )
    return parser
        ...
        json.dumps(
            {
                ...
                "algorithm_lock_created": payload["algorithm_lock_created"],
                "predictor_build_authorized": payload[
                    "predictor_build_authorized"
                ],
                "gshhg_l3_geometry_opened": payload["access_contract"][
