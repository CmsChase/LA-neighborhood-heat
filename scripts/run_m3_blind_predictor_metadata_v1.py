"""CLI for the authorized M3 blind-predictor metadata bootstrap."""
...
from la_heat.multicity.m3_blind_predictor_metadata_v1 import (
    ...
    authenticate_runtime_authorization,
    build_runtime_authorization,
    create_runtime_authorization,
    run_metadata_bootstrap,
)
...
def main() -> None:
    ...
    if args.preview_authorization:
        payload = build_runtime_authorization(args.project_root)
    elif args.create_authorization:
        payload = create_runtime_authorization(args.project_root)
    elif args.check_authorization:
        payload = authenticate_runtime_authorization(args.project_root)
    elif args.run:
        payload = run_metadata_bootstrap(args.project_root)
    else:
        payload = authenticate_metadata_completion(args.project_root)
