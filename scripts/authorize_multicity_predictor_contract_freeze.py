"""Create or authenticate the narrow multicity planning transition v8."""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--check-only",
        action="store_true",
        help="Authenticate the existing canonical v8 planning record.",
    )
    return parser
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    payload = authorize_multicity_predictor_contract_freeze(
        ...
        json.dumps(
            {
                ...
                "water_distance_source_locked": payload["locks"][
                    "portable_water_distance_source_locked"
                ],
                "water_distance_algorithm_locked": payload["locks"][
                    "portable_water_distance_algorithm_locked"
                ],
                "freeze_decision_authorized": payload["authorized_now"][
                    "portable_predictor_source_freeze"
                ],
                "predictor_contract_freeze_authorized": payload[
                    "authorized_now"
