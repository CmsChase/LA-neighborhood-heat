...
| 8. Cross-city continuation — draft; Phoenix geography pilot complete | continuation | Test zero-shot transfer from LA to Phoenix, Houston, and Chicago; add calibrated uncertainty and abstention | Separate protocol/config, city manifests, target-blind predictors, frozen predictions, one-time external evidence | External targets stay sealed until portable sources, remaining metadata adapters, model, thresholds, and output contract are frozen | Research engineering implements; independent review challenges; student approves freeze |
